	...
	<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
			...
			>
				...
				<div class="p-4">
					<div class="flex items-start justify-between gap-2">
						...
						<Badge variant="secondary" class="shrink-0 text-xs">Manage</Badge>
					</div>
					{#if mod.description}
						<p class="mt-1 line-clamp-2 text-xs text-muted-foreground">{mod.description}</p>
					{/if}
				</div>
			</a>
		{/each}
		...
		{#if isSuperAdmin}
			<Button
				href="/modules/create"
				variant="ghost"
				class="min-h-[160px] w-full gap-1.5 rounded-lg border border-dashed text-muted-foreground hover:border-foreground/30 hover:text-foreground"
			>
				<Plus class="size-4" />
				Create Module
			</Button>
